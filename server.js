...
var resp=res;
  ...
  if(req.session.user){
  db.collection("pinterest").findOne({_id:req.query.id}, function(err, doc) {
    if (err) throw err;
    console.log(doc);
    ...
    for(var i=0;i<doc.likes.length;i++){
      ...
      if(doc.likes.length==0){
        ...
        db.collection("piterest").update({_id: new ObjectId(req.query.id)},{$set:  {likes:doc.likes}},  function(err, res) {
        if (err) throw err;
        //console.log(res);
