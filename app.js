    ...
    return (<div className='grid'>
      {this.state.data.map(function(item) {
        ...
        var h=parseInt(item.size.split("x")[1]);
          ...
          <button onMouseOver={th.over} id={item._id} onClick={th.like} className={"lk hid btn btn-default btn-sm glyphicon glyphicon-star "+(user?"":"disabled")}>{item.likes.length}</button>
